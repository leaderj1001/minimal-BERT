...
def _eval(epoch, test_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        mlm_acc /= step
        nsp_acc /= step
        print('[Test Epoch: {}] mlm loss: {}, nsp loss: {}, loss: {}, mlm acc: {}, nsp acc: {}'.format(epoch, mlm_losses,
                                                                                                  nsp_losses, losses,
                                                                                                  mlm_acc, nsp_acc))
...
def _train(epoch, train_loader, model, optimizer, criterion, args):
    ...
    mlm_acc /= step
    nsp_acc /= step
    print('[Train Epoch: {0}] mlm loss: {1:.3f}, nsp loss: {2:.3f}, loss: {3:.3f}, mlm acc: {4:.4f}, nsp acc: {5:.4f}'.format(epoch, mlm_losses, nsp_losses, losses, mlm_acc, nsp_acc))
