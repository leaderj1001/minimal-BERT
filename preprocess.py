...
class Vocab(object):
    def __init__(self, args, corpus_filename=None):
        self.filename = '{}/corpus.pkl'.format(args.base_dir)
        if not os.path.isfile(self.filename):
            ...
            data = pd.read_csv('{}/{}'.format(args.base_dir, corpus_filename), delimiter='\t', header=0)
...
def prepare_dataset(args, mode='train'):
    ...
    if not os.path.isfile(filename):
        ...
        handle_pickle(args, pkl_data, mode=mode)
    else:
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = [self.args.vocab['stoi']['<CLS>']] + rm_s1 + [self.args.vocab['stoi']['<SEP>']] + rm_s2
        label = [self.args.vocab['stoi']['<CLS>']] + rm_s1_label + [self.args.vocab['stoi']['<SEP>']] + rm_s2_label
...
def load_data(args):
    vocab = Vocab(args, corpus_filename='train.tsv')
